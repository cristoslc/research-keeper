...
class TestStalePendingDirs:
    def test_detects_stale_tag_sidecar(self, rk_root: Path):
        ...
        (src_dir / "manifest.yaml").write_text(yaml.dump({
            "slug": "old-source", "kind": "source", "hash": "abc",
        ...
        assert "stale_sidecars" in checks

    def test_fresh_sidecar_not_flagged(self, rk_root: Path):
        """Doctor should not flag recently created sidecars."""
        src_dir = rk_root / "library" / "sources" / "new-source"
        src_dir.mkdir(parents=True)
        (src_dir / "source.md").write_text("content")
        (src_dir / "manifest.yaml").write_text(yaml.dump({
            "slug": "new-source", "kind": "source", "hash": "def",
        ...
        results = run_doctor(rk_root)
        stale_checks = [r for r in results if r.check == "stale_sidecars"]
        assert len(stale_checks) == 0


class TestOrphanedLocks:
    def test_detects_orphaned_resolve_lock(self, rk_root: Path):
        """Doctor should detect .rk-resolve.lock with dead PID."""
        lock_path = rk_root / ".rk-resolve.lock"
        lock_path.write_text("pid: 99999999\nstarted: 2026-03-30T00:00:00Z\n")

        results = run_doctor(rk_root)
        checks = {r.check for r in results}
        assert "orphaned_locks" in checks

    def test_live_resolve_lock_not_flagged(self, rk_root: Path):
        """Doctor should not flag a resolve lock with a live PID."""
        lock_path = rk_root / ".rk-resolve.lock"
        lock_path.write_text(f"pid: {os.getpid()}\nstarted: 2026-03-30T00:00:00Z\n")

        results = run_doctor(rk_root)
        orphan_checks = [r for r in results if r.check == "orphaned_locks"]
...
class TestUnresolvedSidecars:
    def test_reports_unresolved_tag_sidecars(self, rk_root: Path):
        ...
        (src_dir / "source.md").write_text("content")
        (src_dir / "manifest.yaml").write_text(yaml.dump({
            "slug": "unresolved", "kind": "source", "hash": "xyz",
            "freshness": {"ingested": "2026-03-29", "ttl": "30d"},
            "provenance": {"origin": "test"}, "tags": [],
        }))
        (pending / "tag.j2").write_text("{# rk:tag #}\ntags:\n")
    ...
    def test_reports_unresolved_synthesis_sidecars(self, rk_root: Path):
        ...
        (tag_dir / "meta.yaml").write_text(yaml.dump({"slug": "memory", "kind": "tag-synthesis"}))
