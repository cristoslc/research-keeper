...
#### Embeddings moved to Ollama — no more in-process model loading

Embeddings are now generated by Ollama's `/api/embed` endpoint instead
of loading sentence-transformers models in-process. This eliminates OOM
crashes entirely — the ML work runs in the Ollama daemon, not in the `rk`
process. `sentence-transformers`, `torch`, and `transformers` are removed
from dependencies. Config defaults to `provider: ollama` with
`model: nomic-embed-text` (274 MB). A single `build_embedder()` factory
routes all six call sites. `MemoryGuard`, MPS watermark management, and
batch-halving recursion are all removed — none are needed when the
embedding model lives outside the process. `rk init` templates updated
with Ollama install instructions.

### Supporting Changes

- Skill template updated to document Ollama requirements instead of
  sentence-transformers.
- 82-line `OllamaEmbedder` adapter with connection error handling and
  per-item fallback on batch failure.
- 145-line test suite for the Ollama embedder covering byte format,
  truncation, connection errors, and batch/per-item fallback.
- Removed ~460 lines of MPS cache management, MemoryGuard, and
  sentence-transformers adapter code.
- Test fixtures using `provider: none` now route through a noop stub
  in the factory.
